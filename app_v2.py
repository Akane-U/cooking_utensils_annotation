...
UTENSIL_CATEGORIES = {
    "単体使用が多い容器": (100, 199),
    "組み合わせ使用が多い容器": (200, 299),
    "加熱容器": (300, 399),
    "切る": (400, 499),
    "混ぜる": (500, 599),
    "すくう": (600, 699),
    "すりおろす・漉す・ふるう": (700, 799),
    "伸ばす・塗る": (800, 899),
    "整える": (900, 999),
    "量る・測る": (1000, 1099),
    "包む・覆う・敷く": (1100, 1199),
        ...
        for ws in recipe["world_state_list"]:
            for state in ws["state_list"]:
                ...
                for inter in state.get("utensil_interactions_list", []):
                    inter.pop("_uid", None)
                    for pair in inter.get("action_utensil_pairs", []):
                        pair.pop("_uid", None)
    return result
...
def action_type_select(label: str, key: str, current: str) -> str:
    in_list = current in ACTION_TYPES
    opts = [""] + ACTION_TYPES + [ACTION_OTHER]
    display = current if in_list else (ACTION_OTHER if current else "")
    idx = opts.index(display) if display in opts else 0
...
def cb_add_interaction(ridx, sidx, si):
    ...
    ws["state_list"][si]["utensil_interactions_list"].append(
        {"source_state_id": "", "action_utensil_pairs": [], "_uid": uuid.uuid4().hex[:8]}
    )
...
def cb_add_action_pair(ridx, sidx, si, ii):
    ws = get_step_ws(ridx, sidx)
    inter = ws["state_list"][si]["utensil_interactions_list"][ii]
    inter.setdefault("action_utensil_pairs", []).append(
        {"action_type": "", "utensils": [], "_uid": uuid.uuid4().hex[:8]}
    )


def cb_del_action_pair(ridx, sidx, si, ii, pi):
    ws = get_step_ws(ridx, sidx)
    ws["state_list"][si]["utensil_interactions_list"][ii]["action_utensil_pairs"].pop(pi)
    ...
    with nav_col:
        ...
        if st.button("☁ 保存", type="primary", use_container_width=True):
            try:
                ...
                st.success(f"保存しました: outputs/v2/{filename}")
        ...
        if st.button("☁ 読み込み", use_container_width=True):
            try:
                ...
                if loaded is None:
                    st.warning(f"outputs/v2/{filename} がストレージに見つかりません")
                else:
            ...
            with st.container(border=True):
                ...
                for ii, inter in enumerate(interactions):
                    ...
                    with st.container(border=True):
                        ...
                        with del_col:
                            st.write("")
                            if st.button("🗑", key=f"del_{ridx}_{sidx}_{si}_{uid}", help="この生成元を削除"):
                                to_del = ii

                        pairs = inter.setdefault("action_utensil_pairs", [])
                        if not pairs:
                            pairs.append(
                                {"action_type": "", "utensils": [], "_uid": uuid.uuid4().hex[:8]}
                            )
                        ...
                        for pi, pair in enumerate(pairs):
                            puid = pair.setdefault("_uid", uuid.uuid4().hex[:8])
                            wkey = f"u_{ridx}_{sidx}_{si}_{uid}_{puid}"

                            at_col, u_col, copy_col, pdel_col = st.columns([3, 5, 1, 1])

                            with at_col:
                                pair["action_type"] = action_type_select(
                                    "調理動作（action_type）",
                                    f"at_{ridx}_{sidx}_{si}_{uid}_{puid}",
                                    pair.get("action_type", ""),
                                )

                            with u_col:
                                pair["utensils"] = utensil_multi_select(
                                    "使用器具（utensils）※複数選択可",
                                    wkey,
                                    pair.get("utensils", []),
                                    utensil_cats,
                            ...
                            with copy_col:
                                if pi > 0:
                                    prev_utensils = deepcopy(pairs[pi - 1]["utensils"])
                                    ...
                                    def _do_copy(
                                        ...
                                        current_u = _pair.get("utensils", [])
                                        merged = list(dict.fromkeys(current_u + _prev))
                                        _pair["utensils"] = merged
                                    ...
                                    st.button(
                                        "⬆",
                                        key=f"copy_{ridx}_{sidx}_{si}_{uid}_{puid}",
                                        help="1個上の行の器具をコピー",
                                        on_click=_do_copy,
                                    )
                            ...
                            with pdel_col:
                                if len(pairs) > 1:
                                    st.write("")
                                    if st.button("🗑", key=f"delpair_{ridx}_{sidx}_{si}_{uid}_{puid}"):
                                        pair_to_del = (ii, pi)

                        if st.button("＋ action_typeを追加", key=f"addpair_{ridx}_{sidx}_{si}_{uid}"):
                            cb_add_action_pair(ridx, sidx, si, ii)
                            st.rerun()

                if pair_to_del is not None:
                    cb_del_action_pair(ridx, sidx, si, *pair_to_del)
                    st.rerun()
